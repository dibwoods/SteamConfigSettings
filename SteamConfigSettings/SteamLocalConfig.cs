...
namespace SteamConfigHelper {
    public class SteamLocalConfig {
        ...
        public SteamLocalConfig() {
            //Find installed steam location
            RegistryKey key = Registry.CurrentUser;
            using (key = key.OpenSubKey(@"Software\Valve\Steam", false)) {
                steam_location = key.GetValue("SteamPath").ToString();
                foundSteamLocation = true;
            }

            //Throw an exception if we didn't find the steam path
            if (!foundSteamLocation) throw new Exception("Failed to find Steam install location");

            steam_userdata_location = Path.Combine(steam_location, "userdata");
        }
        ...
        private bool SetAppSetting(string _userId, string _appId, string _settingKey, string _settingValue) {
            ...
            if (configData != null && configData.Length > 0) {
                ...
                if (launchOptionsIndex == -1 || launchOptionsIndex >= endIndex) {
                    ...
                    pos += 1;
                    configData = configData.Insert(pos, Environment.NewLine);
